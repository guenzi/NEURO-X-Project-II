...
# =====================================================================
# Traces temporelles (expectation, p_lick, lick, reward, stim1/stim2/RPE, motivation, noise)
# =====================================================================
def plot_traces(time_vect,
    ...
    # --- stimuli (0, 1 or 2)
    stim1 = stim2 = None
    if isinstance(stim_array, (tuple, list)) and len(stim_array) == 2:
        stim1 = _to_1d(stim_array[0])
        stim2 = _to_1d(stim_array[1])
    elif stim_array is not None:
        stim1 = _to_1d(stim_array)  # single channel fallback
    ...
    expc  = _align(expc);  plick = _align(plick); lick  = _align(lick)
    rpe   = _align(rpe);   motiv = _align(motiv); reward = _align(reward)
    stim1 = _align(stim1); stim2 = _align(stim2); noise  = _align(noise)

    # --- unify stim peak height (binary presence)
    s1_plot = (stim1 > 0).astype(float) if stim1 is not None else None
    s2_plot = (stim2 > 0).astype(float) if stim2 is not None else None
    ...
    if (s1_plot is not None) or (s2_plot is not None):
        if s1_plot is not None:
            rows.append(("plot", time_vect, s1_plot, "WDT (Stim1)"))  # blue
        if s2_plot is not None:
            rows.append(("plot", time_vect, s2_plot, "AUD (Stim2)"))  # orange
    else:
    ...
    for ax, (kind, x, y, label) in zip(axs, rows):
        if kind == "plot":
            # color rules for stims
            if label.startswith("WDT"):
                ax.plot(x, y, color="tab:blue")
            elif label.startswith("AUD"):
...
def plot_hr_fa_over_sessions(performance_list: List[np.ndarray],
    ...
    """
    performance_list: liste de tableaux (sortie de function_performance_wdt)
...
def plot_wdt_block_rates(performance: np.ndarray,
    ...
    ax.set_ylim(0, 1)
    ax.set_xlabel("Trial # (stim & catch)")
...
# =====================================================================
def session_rates_by_stim(perf: Optional[np.ndarray],
                          zero_when_empty: bool = True
                         ) -> Tuple[float, float, float]:
    """
    Renvoie (HR_stim1, HR_stim2, FA_catch) pour une session.
    perf colonnes: [t, stim_code(0/1/2), lick, latency, reward, outcome]
      outcome: 0=Miss, 1=Hit, 2=CR, 3=FA
    """
    ...
    # Comptages
    n1, n2, nc = int(np.sum(stim1_mask)), int(np.sum(stim2_mask)), int(np.sum(catch_mask))

    hit_mask = perf[:, 5] == 1
    fa_mask  = perf[:, 5] == 3

    hits1 = int(np.sum(hit_mask & stim1_mask))
    hits2 = int(np.sum(hit_mask & stim2_mask))
    fas   = int(np.sum(fa_mask  & catch_mask))

    # Taux
    def _ratio(num, den):
        if den > 0: return num / den
        return 0.0 if zero_when_empty else float('nan')
...
def plot_session_rates_by_stim(performance_list: Iterable[Optional[np.ndarray]],
                               session_labels: List[str],
                               title: str = "HR par stimulus & FA (catch) — par session",
                               zero_when_empty: bool = True,
                               ax: Optional[plt.Axes] = None
                              ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    Trace, pour chaque session:
      - HR_stim1 (croix bleue)
      - HR_stim2 (croix orange)
      - FA_catch (rond vert)
    Retourne (xs, hr1_arr, hr2_arr, fa_arr).
    """
    ...
    rates = [session_rates_by_stim(p, zero_when_empty=zero_when_empty) for p in perfs]
    ...
    xs = np.arange(1, len(perfs) + 1, dtype=float)
    ...
    # Styles demandés
    ax.plot(xs, hr1_arr, linestyle="None", marker="x", label="HR — stim1 (WDT)", color="tab:blue")
    ax.plot(xs, hr2_arr, linestyle="None", marker="x", label="HR — stim2 (AUD)", color="tab:orange")
    ax.plot(xs,  fa_arr, linestyle="None", marker="o", label="FA — catch",       color="tab:green")
...
def plot_single_session_rates(
    ...
    hr, fa, dprime = session_rates_by_stim(perf, zero_when_empty=zero_when_empty)
...
def plot_multi_mouse_single_session(
    ...
    for i, (perf, label) in enumerate(zip(perfs, mouse_labels)):
        hr, fa, dp = session_rates_by_stim(perf, zero_when_empty=zero_when_empty)
...
# =====================================================================
# Visualisation de la sigmoïde de gain de bruit
# =====================================================================
def plot_noise_gain_sigmoid(m0: float,
    ...
    """
    Trace g(m) = gmin + (gmax - gmin) * 1/(1 + exp(-k*(m - m0))) pour m∈[0,1].
    """
    if m_grid is None:
