...
class Mouse:
    ...
    # Standardisation : tau_nogo reste FIXE (nogo_streak_incr[1]=0 par defaut cote config), et
    # c'est le GAIN No-Go qui apprend a la place — meme regle delta que le gain Go (learning_stim),
    # sur -rpe_t, declenchee a la meme cadence que l'ancienne sensibilisation du tau (tous les
    # nogo_streak_incr[0] bouts non recompenses DANS un essai, pas chaque bout individuel).
    ...
    # Baisse symetrique de stim_gain_right/left quand un lick correct (bon cote, dans la
    # fenetre de reponse, cooldown ecoule) n'est PAS recompense parce que le tirage de
    # reward_prob a echoue (reward_prob=1.0 par defaut hors desapprentissage cible -> ce
    # tirage ne peut alors jamais echouer, le mecanisme reste totalement inerte en
    # entrainement normal). Sans ca, stim_gain reste fige a sa valeur apprise une fois haute
    # et continue d'injecter un boost anticipatoire meme en pleine extinction. Valide : aucun
    # impact sur la regression (courbes identiques au bit pres), extinction plus profonde
    # sur le cote cible (floor ~0.09 vs ~0.17 sans ce mecanisme).
    ...
    # (whisker=1, auditif=2), chacun avec SON PROPRE gain — meme role et meme regle que
    # Mouse.exp_update_stim en mono (monte a la recompense, via learning_stim_wa). Standardise
    # sur le meme principe que Mouse.stim_gain_noreward_active (mono/dual) plutot qu'une voie
    # No-Go competitrice separee : le gain d'un stimulus redescend directement (meme formule
    # que sa propre montee) des qu'un lick sur ce stimulus n'est pas recompense parce que ce
    # n'est plus le stimulus actuellement recompense (mismatch de contingence — l'equivalent
    # whisker/auditif du tirage de probabilite refuse en mono/dual). Toujours actif ici (pas
    # optionnel comme stim_gain_noreward_active) : contrairement au mono/dual, whisker/auditif
    # a TOUJOURS un stimulus non recompense a un instant donne, c'est le coeur du paradigme.
    # Champs suffixes _wa, n'entrent jamais en jeu pour le mono ou le dual gauche/droite.
    exp_update_stim1_wa: tuple = (1, 0.1)       # (tau, gain) whisker
    exp_update_stim2_wa: tuple = (1, 0.1)       # (tau, gain) auditif
    stim_gain_max_wa: float = 8.0               # plafond des deux gains ci-dessus. Volontairement
                                                 # haut : un plafond bas fait saturer le gain des la
                                                 # 1ere session (recompensee), apres quoi il ne peut
                                                 # plus redescendre suffisamment vite apres le switch.
    learning_stim_wa: float = 0.1               # taux d'apprentissage du gain (meme taux dans les
                                                 # deux sens — monte a la recompense, descend au
                                                 # mismatch — comme Mouse.learning_stim en mono).
...
class SimConfig:
    ...
    # Standardisation : tau ET gain No-Go grandissent tous les deux a ce declenchement (le gain
    # seul, teste d'abord, sous-suppime l'extinction par rapport a la combinaison des deux —
    # valide par grid de seeds, aucune perte sur la regression normale).
    NOGO_TAU_GROWTH: float = 1.0        # increment de tau_nogo a chaque seuil atteint (0 = desactive)
    NOGO_TAU_MAX: float = 60.0          # plafond du tau_nogo (inutilise si NOGO_TAU_GROWTH=0)
    LEARNING_NOGO_GAIN: float = 0.05    # taux d'apprentissage du gain No-Go (valide par grid de seeds)
    NOGO_GAIN_MAX: float = 0.6          # plafond du gain No-Go appris

    # Baisse symetrique de stim_gain_right/left sur reward refusee par le tirage de probabilite
    # (voir Mouse.stim_gain_noreward_active) : inerte hors desapprentissage cible, approfondit
    # l'extinction du cote concerne sans toucher a l'autre cote ni a la regression.
    STIM_GAIN_NOREWARD_ACTIVE: bool = True
