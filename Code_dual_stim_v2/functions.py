...
def function_mouse_lick(mouse: Mouse, mouse_session: MouseSessionState, i: int) -> tuple[int, MouseSessionState]:
    ...
    le poids sur l'expectation E diminue (moins d'exploitation).
    ...
    drive = expectation_tm1 * w_exploit + gamma_noise * w_explore
    d_t = drive * (mouse.value * motivation_t - mouse.cost)

    if d_t > mouse.lick_thrs:
        lick = 1

    mouse_session.decision[i, 0] = float(d_t)
    mouse_session.p_lick[i, 0] = float(d_t)   # gardé pour compat avec les plots existants
