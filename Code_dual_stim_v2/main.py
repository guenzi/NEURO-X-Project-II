...
PLOT_TRACES: bool = True                # time-series traces (licks, stim, noise)
...
PLOT_SESSIONS_COMPARISON: bool = True   # compare HR(stim1/stim2) & FA(catch) across sessions
PLOT_SINGLE_SESSION_ALL: bool = True    # HR vs FA + d' for each session individually
PLOT_STIM_GAINS: bool = True            # plot evolution of stimulus gains across sessions
...
# DISABLED: this legacy mechanism modulated mouse.noise[2] (the Nd generator's gain) via its
# own sigmoid on a sliding mean of |RPE| — redundant with, and inconsistent with, the real
# Uncertainty function (U) which now drives the exploitation/exploration weighting directly
# inside D. Nd should stay a CONSTANT-gain generator; U changes the weight, not the amplitude.
USE_SIGMOID_NOISE: bool = False   # enable online sigmoid noise modulation
