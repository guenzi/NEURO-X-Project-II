...
# ------------------------------ Désapprentissage (architecture Go/No-Go) ------------------------------
DELEARNING: bool = False    # si True: en plus de la dynamique actuelle de E_go (inchangee),
                            # les bouts de leche non recompenses alimentent une voie E_nogo
                            # separee, plus lente/persistante, qui s'oppose a E_go a la decision
NOGO_TAU: float = 8.0       # constante de temps de decroissance de E_nogo entre deux bouts (s)
NOGO_GAIN: float = 0.15     # fraction de (1 - E_nogo) ajoutee a chaque bout non recompense
NOGO_RELIEF: float = 0.4    # fraction de E_nogo effacee a chaque recompense confirmee
...
_results_suffix += "_delearn" if DELEARNING else ""
...
mouse = Mouse(reward_value=REWARD_VALUE, cost=COST, lick_thrs=LICK_THRS_FL, lick_thrs_wdt=LICK_THRS_WDT,
              delearning_enable=DELEARNING, exp_update_nogo=(NOGO_TAU, NOGO_GAIN), nogo_relief=NOGO_RELIEF)
...
init_nogo = float(log_mouse_fl1.expectation_nogo[-1, 0])  # E_nogo persiste aussi entre sessions

fl_session   = FreeLickingSessionState(); fl_session.initialize(session_info.number_bin, fl_params.no_lick_wind)
mouse_session = MouseSessionState();      mouse_session.initialize(session_info.number_bin, mouse.motivation[0], init_expect, init_uncert, init_nogo)
...
for s_idx in range(1, NUM_WDT + 1):
    ...
    init_nogo = float(prev_ms.expectation_nogo[-1, 0])  # E_nogo persiste aussi entre sessions
...
wdt_test_mouse   = MouseSessionState(); wdt_test_mouse.initialize(session_info.number_bin, mouse.motivation[0], init_expect, init_uncert, init_nogo)
...
if SAVE_PARAMETERS_TXT:
    ...
    param_rows = [
        ...
        ("Enabled", DELEARNING),
        ("Tau (no-go, s)", NOGO_TAU),
        ("Gain (no-go)", NOGO_GAIN),
        ("Relief (fraction effacee par recompense)", NOGO_RELIEF),
