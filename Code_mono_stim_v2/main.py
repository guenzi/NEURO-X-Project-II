# Deroulement : 2 sessions Free Licking + N sessions WDT (+ test psychometrique en mono/dual).
...
# La decision de base utilise toujours E_go - E_nogo (architecture Go/No-Go, plus optionnelle),
# a l'identique dans les trois modes (une voie No-Go par cote en dual gauche/droite, une voie
# No-Go par stimulus en whisker/auditif, une voie generale en mono). Pour observer un
# desapprentissage sur un run normal (mono/dual), fixer DELEARNING_FROM_SESSION (ex. 6) : plus
# aucune recompense a partir de WDT6 (DELEARNING_SIDE cible un seul cote en dual), et 3 plots
# de diagnostic en plus des plots habituels.
# Pour comparer une population de souris (mono/dual, learning_stim/noise tires +/- autour de la
# base, pas de plots individuels), fixer POPULATION_RANGE=True et POPULATION_N_MICE (non
# supporte en whisker/auditif).
...
config = SimConfig(WHISKER_AUD_STIM=True)
